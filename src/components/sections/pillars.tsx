...
import {
  ...
  OriginPanel,
  VerdictPanel,
} from "@/components/mockup/pillar-panels";
  ...
  return (
    <section className="py-[clamp(4rem,9vw,7rem)]">
      <Container>
        ...
        <div className="mt-12 grid grid-cols-1 border-t border-border md:grid-cols-3">
          {pillars.map((pillar) => (
            <article
              ...
              className="flex flex-col border-b border-border py-8 md:border-b-0 md:border-r md:px-8 md:first:pl-0 md:last:border-r-0 md:last:pr-0"
            >
              <p className="font-mono text-label uppercase text-muted-foreground">
                {pillar.capability}
              </p>
              ...
              {/* Pushed to the bottom of the column so the three panels line up
                  across the row even though the copy above them is uneven. */}
              <div className="mt-auto pt-8">
                {pillar.panel}
                <p className="mt-6 border-l border-border-control pl-4 text-sm text-foreground">
                  {pillar.aside}
                </p>
              </div>
            </article>
        ...
        <p className="mt-8 max-w-[68ch] text-sm text-muted-foreground">
          The three panels above are illustrations of the design, drawn to explain the idea.
          Endpoint Forms is pre-launch, so every number in them is invented &mdash; none of
          it is a measured result and none of it is a screenshot.
        </p>
      </Container>
    </section>
  );
